...
func _physics_process(delta: float):
	...
	_steer_target = Input.get_axis(&"driving_right", &"driving_left")
	...
	if DisplayServer.is_touchscreen_available() or Input.is_action_pressed(&"driving_accelerate"):
		...
		if not DisplayServer.is_touchscreen_available():
			...
			engine_force *= Input.get_action_strength(&"driving_accelerate")
	else:
		engine_force = 0.0

	if Input.is_action_pressed(&"driving_reverse"):
		...
		engine_force *= Input.get_action_strength(&"driving_reverse")
